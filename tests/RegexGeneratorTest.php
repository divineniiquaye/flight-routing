...
class RegexGeneratorTest extends TestCase
{
    ...
    {
        $collection = new RegexGenerator('/');

        foreach ($routes as $route) {
            [$path, $name] = $route;
            $collection->addRoute($path, [$name]);
        }
...
EOF
    ...
    private function dumpCollection(RegexGenerator $collection, $prefix = '')
    {
        $lines = [];

        foreach ($collection->getRoutes() as $item) {
            if ($item instanceof RegexGenerator) {
