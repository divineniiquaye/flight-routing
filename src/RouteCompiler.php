...
class RouteCompiler
{
    ...
    public function __construct(Route $route, Request $request, array $parameters)
    {
        $this->requirements = $parameters;
        $this->setPath($route->getUri(), $route->getDefault(null), $request);

        $this->compile($route); // Compile Route

        unset($this->tokens); // Nothing special.
    }
     ...
     * @return $this
     */
    protected function setPath(string $pattern, array $defaults, Request $request)
    {
        // A pattern must start with a slash and must not have multiple slashes at the beginning because the
        // generated path for this route would be confused with a network path, e.g. '//domain.com/path',
        // which may cause problems in Linux servers.
        $uri = $this->resolveUri($pattern, $request);

        // Place Defaults on Uri.
        $uri = preg_replace_callback('/\??\{(.*?)\??\}/', function ($match) use (&$defaults) {
            return isset($defaults[$match[1]]) ? $defaults[$match[1]] : $match[0];
        }, $uri);

        $this->path = str_replace("\\", '/', $uri);
    ...
    protected function compile(Route $route)
    {
        ...
        $result = $this->compilePattern($route, $this->path, false);
        $pathVariables = $result['variables'];

        foreach ($pathVariables as $pathParam) {
            if ('_fragment' === $pathParam) {
                throw new \InvalidArgumentException(sprintf('Route pattern "%s" cannot contain "_fragment" as a path parameter.', $route->getUri()));
            }
        }
     ...
     * @return string
     */
    private function resolveUri(string $uri, Request $request)
    {
        $uri = trim($uri, '/');
        $fullPath = dirname($request->getServerParams()['SCRIPT_NAME']);

        if (mb_strlen($fullPath) > 1) {
            return $fullPath.'/'.$uri;
        }
    ...
    private function compilePattern(Route $route, string $pattern, bool $isHost): array
    {
        $tokens = [];
        $variables = [];
        $matches = [];
        ...
        if (!$needsUtf8 && $useUtf8 && preg_match('/[\x80-\xFF]/', $pattern)) {
            throw new \LogicException(sprintf('Cannot use UTF-8 route patterns without setting the "utf8" option for route "%s".', $route->getUri()));
        }
        ...
        foreach ($matches as $match) {
            ...
            $regexp = $this->requirements[$varName] ?? '[^/]++';
            if (null === $regexp) {
     ...
     */
    private function determineStaticPrefix(Route $route, array $tokens): string
    {
        if ('text' !== $tokens[0][0]) {
